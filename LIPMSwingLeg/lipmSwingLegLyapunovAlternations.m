function [ V,B ] = lipmSwingLegLyapunovAlternations(x,f,g,V0,B0,R)
if nargin < 6
  R = [];
end
...
for i=1:max_iter
  ...
  for j=1:2^nU
    ...
    for k=1:nU
      [prog, Vdot_sos,bmult{j}{k},coeff] = spotless_add_sprocedure(prog, Vdot_sos, umat(j,k)*B(k),x,0,4);
    end
...
%   solver = @spot_sedumi;
  ...
  if double(sol.eval(gamma)) < -1e-6% && sol.status == spotsolstatus.STATUS_PRIMAL_AND_DUAL_FEASIBLE
    ...
    Q_opt = sol.eval(Q);
    is_fail = false;
    fprintf("%f", double(sol.eval(cost)));
  else
    cost_min = cost_val;
    if is_fail,
      keyboard
