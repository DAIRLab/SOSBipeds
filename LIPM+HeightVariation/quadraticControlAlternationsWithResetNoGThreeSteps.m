...
for i=1:1,
  ...
  if ~sample_time
    [prog, Vdot_sos] = spotless_add_sprocedure(prog, Vdot_sos, t*(T-t),sproc_vars,0,4);
  end
  ...
  for i=1:length(u),
    [prog, up_sos,upmult{i}] = spotless_add_sprocedure(prog, 1-u(i), rho-V,sproc_vars,0,2);
    [prog, um_sos,ummult{i}] = spotless_add_sprocedure(prog, u(i)+1, rho-V,sproc_vars,0,2);
    
    if ~sample_time
      [prog, up_sos] = spotless_add_sprocedure(prog, up_sos, t*(T-t),sproc_vars,0,2);
      prog = prog.withSOS(up_sos - 0*gamma*(x_mss'*x_mss)^2);
      
      [prog, um_sos] = spotless_add_sprocedure(prog, um_sos, t*(T-t),sproc_vars,0,2);
  ...
  [prog, rad_sos,rad_mult] = spotless_add_sprocedure(prog, (b^2-4*a*d)*(1+x_mss'*x_mss), subs(rho-V,t,T),x_mss,0,2);
  [prog, rad_sos] = spotless_add_sprocedure(prog, rad_sos, b,x_mss,0,2);
  prog = prog.withSOS(rad_sos);  
  
  [prog, ab_sos,ab_mult] = spotless_add_sprocedure(prog, (2*a-b)*(1+x_mss'*x_mss), subs(rho-V,t,T),x_mss,0,2);
  [prog, ab_sos] = spotless_add_sprocedure(prog, ab_sos, b,x_mss,0,2);
