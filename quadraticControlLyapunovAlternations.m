...
for i=1:1,
  ...
  [prog, Vdot_sos,mult,coeff] = spotless_add_sprocedure(prog, -Vdot, 1-V,x_mss,2,4);

  prog = prog.withSOS(Vdot_sos - gamma*(x_mss'*x_mss)^3);
  for i=1:length(u),
    [prog, u_sos,upmult{i}] = spotless_add_sprocedure(prog, 1-u(i), 1-V,x_mss,0,2);
    prog = prog.withSOS(u_sos);
    [prog, u_sos,ummult{i}] = spotless_add_sprocedure(prog, u(i)+1, 1-V,x_mss,0,2);
    prog = prog.withSOS(u_sos);
  end
