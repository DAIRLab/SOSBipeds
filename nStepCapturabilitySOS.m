...
if options.control_design
    ...
    for i=1:model.num_inputs,
      ...
      [prog, p_pos_sos_i] = spotless_add_sprocedure(prog, p_pos_sos_i, h_X,V_vars,0,Vdot_degree-2);
      if time_varying
        [prog, p_pos_sos_i] = spotless_add_sprocedure(prog, p_pos_sos_i,  t * (T - t),V_vars,0,Vdot_degree-2);
      end
      ...
      [prog, p_neg_sos_i] = spotless_add_sprocedure(prog, p_neg_sos_i, h_X,V_vars,0,Vdot_degree-2);
      if time_varying
        [prog, p_neg_sos_i] = spotless_add_sprocedure(prog, p_neg_sos_i,  t * (T - t),V_vars,0,Vdot_degree-2);
      end
      ...
      if ~options.korda_control_design
        [prog, p_sos_i] = spotless_add_sprocedure(prog, p, h_X,V_vars,0,Vdot_degree-2);
        if time_varying
          [prog, p_sos_i] = spotless_add_sprocedure(prog, p_sos_i,  t * (T - t),V_vars,0,Vdot_degree-2);
        end
